...
fn begin_benchmark(config: &SimConfig, now: f64, runner: &mut BenchmarkRunner) -> Result<()> {
    ...
    for repeat in 1..=config.benchmark.repeats_per_condition.max(1) {
        for &distance_m in &config.benchmark.distances_m {
            for &angular_speed_rad_s in &config.benchmark.angular_speeds_rad_s {
...
fn apply_current_case(
    ...
    target.angular_speed_rad_s = case.angular_speed_rad_s;
...
fn finalize_current_case(
    ...
    let result = TrialResult {
        ...
        angular_speed_rad_s: case.angular_speed_rad_s,
...
fn write_outputs(config: &SimConfig, dir: &Path, results: &[TrialResult]) -> Result<()> {
    ...
    write_angular_speed_degradation(dir, results)?;
...
fn write_conditions(dir: &Path, results: &[TrialResult]) -> Result<()> {
    ...
    for ((distance, angular_speed, speed), group) in groups {
        ...
        w.serialize(ConditionRow {
            ...
            angular_speed_rad_s: f32::from_bits(angular_speed),
...
struct AngularSpeedRow {
    angular_speed_rad_s: f32,
    trials: usize,
    hit_rate_pct: f32,
    hit_rate_degradation_vs_stationary_pct: Option<f32>,
    mean_dps: f32,
    dps_degradation_vs_stationary_pct: Option<f32>,
    kill_success_rate_pct: f32,
    mean_kill_time_s: Option<f64>,
    timeout_rate_pct: f32,
}

fn write_angular_speed_degradation(dir: &Path, results: &[TrialResult]) -> Result<()> {
    let mut groups: BTreeMap<u32, Vec<&TrialResult>> = BTreeMap::new();
    for r in results {
        groups
            .entry(r.angular_speed_rad_s.to_bits())
            .or_default()
            .push(r);
    }

    let mut rows = Vec::new();
    for (angular_speed_bits, group) in groups {
        let n = group.len() as f32;
        let kill_times: Vec<f64> = group.iter().filter_map(|r| r.kill_time_s).collect();
        rows.push(AngularSpeedRow {
            angular_speed_rad_s: f32::from_bits(angular_speed_bits),
            trials: group.len(),
            hit_rate_pct: group.iter().map(|r| r.hit_rate_pct).sum::<f32>() / n,
            hit_rate_degradation_vs_stationary_pct: None,
            mean_dps: group.iter().map(|r| r.effective_dps).sum::<f32>() / n,
            dps_degradation_vs_stationary_pct: None,
            kill_success_rate_pct: group.iter().filter(|r| r.killed).count() as f32 * 100.0 / n,
            mean_kill_time_s: if kill_times.is_empty() {
                None
            } else {
    ...
    rows.sort_by(|a, b| a.angular_speed_rad_s.total_cmp(&b.angular_speed_rad_s));

    if let Some(base) = rows
        .iter()
        .find(|r| r.angular_speed_rad_s.abs() < 1e-4)
        .cloned()
    {
        for row in &mut rows {
            row.hit_rate_degradation_vs_stationary_pct =
                degradation(base.hit_rate_pct, row.hit_rate_pct);
            row.dps_degradation_vs_stationary_pct = degradation(base.mean_dps, row.mean_dps);
        }
    }

    let mut w = csv::Writer::from_path(dir.join("angular_speed_degradation.csv"))?;
    for row in rows {
        w.serialize(row)?;
    }
    w.flush()?;
